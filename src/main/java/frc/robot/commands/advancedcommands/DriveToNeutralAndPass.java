 ...
 */
public class DriveToNeutralAndPass extends SequentialCommandGroup {

    public DriveToNeutralAndPass(Drivetrain drivetrain, Shooter shooter,
        ...
        addCommands(
                ...
                new DriveDistanceWithPID(drivetrain, 3.75, DriveDirection.X, false), // drive to neutral zone
                new DriveDistanceWithPID(drivetrain, 2, DriveDirection.Y, false), // drive to fuel level

                new SwerveRotateWithPID(drivetrain, () -> 90.0, false), // rotate to intake

                new ParallelDeadlineGroup( // keep collecting until drove the entire way
                        new Collect(collection, collectionMovement),
                        new DriveDistanceWithPID(drivetrain, 1, DriveDirection.X, false)
                ),

                new DriveDistanceWithPID(drivetrain, -1, DriveDirection.X, false), // drive to passing position
                new SwerveRotateWithPID(drivetrain, () -> null, false), // rotate to shoot
                new AccelerateAndShoot(shooter, () -> null, spinningMagazine, kicker)
                        .withTimeout(-1) // shoot until all balls are shot
